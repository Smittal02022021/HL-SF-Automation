...
{
    class T2358AndT2359_AddPrimaryAndBillingContactWithoutEmailAndTitledetailsAndVerifyBillingRequestOnCFAndFASEngagement_Lightning : BaseClass
    {
        ...
        public void HLPostTransactionOpp_FieldsAndValues()
        {
            try
            {
                ...
                for (int row = 2; row <= rowJobType; row++)
                {
                    ...
                    engHome.SearchEngagementWithNumberOnLightning(engName, ReadExcelData.ReadData(excelPath, "Engagement", 6));

                    //Update the Primary and billing contacts for that email and title fields are empty
                    string value = engagementDetails.UpdateEngContactL(ReadExcelData.ReadData(excelPath, "Engagement", 2), LOB, ReadExcelData.ReadData(excelPath, "Engagement", 7));
                    Console.WriteLine("value: " + value);
                    if (valUser.Equals("James Craven"))
                       { 
                    Assert.AreEqual("Ms. " + ReadExcelData.ReadData(excelPath, "Engagement", 2) + ReadExcelData.ReadData(excelPath, "Engagement", 7), value);
                    extentReports.CreateLog("Primary and billing contacts is updated such that email and title fields are empty ");
                       }

                    else
                    {
                        Assert.AreEqual(ReadExcelData.ReadData(excelPath, "Engagement", 2) + ReadExcelData.ReadData(excelPath, "Engagement", 7), value);
                        extentReports.CreateLog("Primary and billing contacts is updated such that email and title fields are empty ");
                    }

                    //Click on Billing request button and validate error message
                    engagementDetails.ClickBillingRequestButtonL();
                    string contact_validation = engagementDetails.GetContactValidationMessageL();
                    Console.WriteLine(contact_validation);
                    ...
                    string updatedValue = engagementDetails.UpdateEngContactL(ReadExcelData.ReadData(excelPath, "Engagement", 3), LOB, ReadExcelData.ReadData(excelPath, "Engagement", 8));
                    Console.WriteLine("updatedValue: " + updatedValue);
                    if (valUser.Equals("James Craven"))
                    {
                        Assert.AreEqual("Ms. " + ReadExcelData.ReadData(excelPath, "Engagement", 3) + ReadExcelData.ReadData(excelPath, "Engagement", 8), updatedValue);
                    ...
                    engagementDetails.ClickBillingRequestButtonL();
                    string titleBilling = engagementDetails.GetTitleOfSendEmailButtonL();
