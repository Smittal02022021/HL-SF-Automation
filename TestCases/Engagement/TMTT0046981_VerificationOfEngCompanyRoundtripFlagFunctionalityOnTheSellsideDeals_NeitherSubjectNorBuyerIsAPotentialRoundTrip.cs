...
{
    class TMTT0046981_VerificationOfEngCompanyRoundtripFlagFunctionalityOnTheSellsideDeals_NeitherSubjectNorBuyerIsAPotentialRoundTrip : BaseClass
    {
        ...
        public static string fileTMTT0046981 = "TMTT0046981_VerificationOfEngCompanyRoundtripFlagFunctionalityOnTheSellsideDeals_NeitherSubjectNorBuyerIsAPotentialRoundTrip";
        ...
        [Test]
        public void VerificationOfEngCompanyRoundtripFlagFunctionalityOnTheSellsideDeals_NeitherSubjectNorBuyerIsAPotentialRoundTrip()
        {
            try
            {
                ...
                for(int row = 4; row <= rowCount; row++)
                {
                    ...
                    //Get Counterparty company ownership and type
                    string counterpartyCompOwnership = lvEngagementDetails.GetCounterpartyCompanyOwnership(counterpartyCompanyNameExl);
                    string buyerCompType = lvEngagementDetails.GetCounterpartyCompanyType(counterpartyCompanyNameExl);

                    //Get Subject Company Type
                    string subCompType = lvEngagementDetails.GetSubjectCompanyType(valSubjectCompName);

                    //Check Subject Company Type 
                    if(subCompType == "Operating Company" && buyerCompType == "Operating Company" && counterpartyCompOwnership == "Private Equity Group" || counterpartyCompOwnership == "Family Office" || counterpartyCompOwnership == "Hedge Fund" || counterpartyCompOwnership == "Institutional Debt Holder")
                    {
                        //TMTI0116113 - Verify that if the user selects "Neither subject nor buyer are round trip" in  Engagement is a Potential Round Trip AND 'SUBJECT' is OpCo & 'COMPANY CLOSED WITH' is PE or PE Owned, prompt will appear and set the values as selected. 
                        ...
                        extentReports.CreateStepLogs("Passed", "Warning Message is Displayed when user selects: Neither subject nor buyer are round trip under Engagement is a Potential Round Trip field when Subject Company = Operating Company & 'COMPANY CLOSED WITH' = Private Equity Group.");
                        ...
                        string fReasonComment1 = ReadExcelData.ReadData(excelPath, "FlagReason", 2);
                        ...
                        string fReason2 = ReadExcelData.ReadData(excelPath, "FlagReason", 1);
                        string fReasonComment2 = ReadExcelData.ReadData(excelPath, "FlagReason", 3);
                    ...
                    else
                    {
                        if(subCompType != "Operating Company" && counterpartyCompOwnership != "Private Equity Group" || counterpartyCompOwnership != "Family Office" || counterpartyCompOwnership != "Hedge Fund" || counterpartyCompOwnership != "Institutional Debt Holder")
                        {
                            ...
                            extentReports.CreateStepLogs("Passed", "No Warning Message is Displayed when user selects: Neither subject nor buyer are round trip under Engagement is a Potential Round Trip field when Subject Company != Operating Company & 'COMPANY CLOSED WITH' != Private Equity Group.");
                        ...
                        if (subCompType == "Operating Company" && buyerCompType != "Operating Company" && counterpartyCompOwnership == "Private Equity Group" || counterpartyCompOwnership == "Family Office" || counterpartyCompOwnership == "Hedge Fund" || counterpartyCompOwnership == "Institutional Debt Holder")
                        {
                            //TMTI0116121 - Verify that if the user selects "Neither subject nor buyer are round trip" in Engagement is a Potential Round Trip AND 'SUBJECT' is OpCo & 'COMPANY CLOSED WITH' is NOT OpCo and NOT PE or PE Owned, prompt will appear and set the values as selected. 

                            //Verify warning message is displayed
                            lvEngagementDetails.SelectValueInPotentialRoundTripField("Neither subject nor buyer are round trip");
                            Assert.IsTrue(lvEngagementDetails.VerifyNeitherBuyerNorSubjectWarningMsgIsDisplayed());
                            extentReports.CreateStepLogs("Passed", "Warning Message is Displayed when user selects: Neither subject nor buyer are round trip under Engagement is a Potential Round Trip field when Subject Company = Operating Company & 'COMPANY CLOSED WITH' != Private Equity Group & 'COMPANY CLOSED WITH' != Operating Company.");
                            ...
                            string fReasonComment5 = ReadExcelData.ReadData(excelPath, "FlagReason", 2);
