        ...
        public void VerifyRateGenerationToStaffMemberAreDeterminedThroughTheirTitles()
        {
            try
            {
                ...
                for (int row = 2; row <= rowRateSheet; row++)
                {
                    ...
                    //Verify selected activity
                    string valueActivity = timeEntry.GetSelectedActivity();
                    string valueActivityExl = ReadExcelData.ReadData(excelPath, "SummaryLogs", 3);
                    ...
                    if (valueProjectOrEngagmentExl.Contains(valueProjectOrEngagmentInDetailLog))
                    {
                    ...
                    //Verify selected activity
                    string valueActivityInDetailLog = timeEntry.GetSelectedActivityOnDetailLog();
                    string valueActivityInExlDetailLogs = ReadExcelData.ReadData(excelPath, "DetailLogs", 3);
                    Assert.AreEqual(valueActivityInExlDetailLogs, valueActivityInDetailLog);
