    ...
    class TC2209AndT2210_PortfolioValuation_ValidateRevenueFieldsUpdateWithStatusInProgress_CompletedAndPositionCancelled : BaseClass
    {
        ...
        public void ValidateRevenueFieldsUpdate()
        {
            try
            {
                ...
                Assert.AreEqual(DateTime.Now.ToString("M/d/yyyy", CultureInfo.InvariantCulture), compDate.Substring(0,10));
                extentReports.CreateLog("Completed Date: " + compDate.Substring(0,10) + " same as today's date is displayed ");
                ...
                Assert.AreEqual(DateTime.Now.ToString("M/d/yyyy", CultureInfo.InvariantCulture), can_Date.Substring(0,10));
                extentReports.CreateLog("Cancel Date: " + can_Date.Substring(0,10) + " same as today's date is displayed after position is cancelled ");
