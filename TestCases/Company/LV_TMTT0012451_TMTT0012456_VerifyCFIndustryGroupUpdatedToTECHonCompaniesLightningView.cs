            ...
            try
            {
                ...
                for (int record = 2; record <= countCompanyRecordTypeExl; record++)
                {
                    string companyRecordTypeExl = ReadExcelData.ReadDataMultipleRows(excelPath, "CompanyType", record, 1);
                     companyName = companyDetails.CreateCompanyLV(companyRecordTypeExl);
                    ...
                    Assert.IsTrue(companyDetails.IsIndustryTypePresentonCoverageTeamLV(industryGroupExl), "Verify Industry Group is updated on Company's Coverage Team under Type Drop-Down ");
